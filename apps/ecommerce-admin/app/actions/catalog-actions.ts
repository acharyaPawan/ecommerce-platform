...
import {
  getServiceAuthTokenFromRequest,
...
export async function seedRandomProductsAction(
  ...
  if (!token) {
    return errorState(seedInitialState, "Authentication required.")
  }
...
export async function createCatalogProductAction(
  ...
  const token = await getServiceAuthTokenFromRequest()
  if (!token) {
    return errorState(createProductInitialState, "Authentication required.")
  }
...
export async function updateCatalogProductAction(
  ...
  const token = await getServiceAuthTokenFromRequest()
  if (!token) {
    return errorState(updateProductInitialState, "Authentication required.")
  }
