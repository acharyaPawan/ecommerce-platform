...
export async function setupServer(app: AppType, port?: number): Promise<ServerType> {
  return serve({
    fetch: app.fetch,
    port: port ?? Number(process.env.PORT) ?? 3001,
  });
}

export async function importFreshDb(): Promise<{
  db: typeof import("../../src/db").default;
  pool: Pool;
}> {
  const dbModule = await import("../../src/db");
  return { db: dbModule.default, pool: dbModule.pool };
}
