...
import { paymentsOutboxEvents } from "../db/schema.js";
import logger from "../logger.js";

type PaymentsOutboxRecord = typeof paymentsOutboxEvents.$inferSelect;

const DEFAULT_BATCH_SIZE = 25;
const DEFAULT_POLL_INTERVAL_MS = 1000;
const WORKER_NAME = "[payments-outbox-worker]";
...
export class PaymentsOutboxPublisherWorker {
  ...
  private async publishNextBatch(): Promise<number> {
    const pendingEvents = await db
      .select()
      .from(paymentsOutboxEvents)
      .where(eq(paymentsOutboxEvents.status, "pending"))
      .orderBy(asc(paymentsOutboxEvents.occurredAt))
  ...
  private async claim(id: string): Promise<boolean> {
    const result = await db
      .update(paymentsOutboxEvents)
      .set({
        status: "processing",
        updatedAt: new Date(),
        error: null,
      })
      .where(and(eq(paymentsOutboxEvents.id, id), eq(paymentsOutboxEvents.status, "pending")))
      .returning({ id: paymentsOutboxEvents.id });
  ...
  private async markPublished(id: string): Promise<void> {
    await db
      ...
      .where(eq(paymentsOutboxEvents.id, id));
  }
...
function mapToIntegrationEvent(record: PaymentsOutboxRecord): EventEnvelope<Record<string, unknown>> {
  return {
    id: record.id,
    type: record.type,
    occurredAt:
...
export async function runPaymentsOutboxPublisherWorker(): Promise<void> {
  const broker = await RabbitMqClient.connect({
    url: process.env.RABBITMQ_URL,
    exchange: process.env.ORDER_EVENTS_EXCHANGE ?? "orders.events",
    exchangeType: "topic",
    queue: process.env.ORDER_EVENTS_QUEUE ?? "payments.events.publisher",
    prefetch: 0,
  });

  const worker = new PaymentsOutboxPublisherWorker(broker, {
    batchSize: resolveNumberFromEnv("PAYMENTS_OUTBOX_BATCH_SIZE", DEFAULT_BATCH_SIZE),
    pollIntervalMs: resolveNumberFromEnv("PAYMENTS_OUTBOX_POLL_INTERVAL_MS", DEFAULT_POLL_INTERVAL_MS),
  });
